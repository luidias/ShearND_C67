...
This directory contains sub-directories for each specimen (H1, H2, etc.), which in turn contain .csv files for individual tests on the given specimen. There are six files per specimen, one for each test condition (Anterior/posterior directions and 1, 10, and 100 mm/s). Test condition is reflected in the file name, in the format <ID>_<Level>_<Direction>_.

These files contain the 3D displacements, rotations, and loads that were experienced by the FSU under the respective test condition. Axes were aligned with the anatomical axes:
...
- Columns 4-6 are the X, Y, Z rotations fo the same, measured in degrees.
...
- **ID**: Identifier code for the specimen. Matches corresponding folder in load-displacement data.
...
- **Balancing_Mass_kg**: Counterbalance mass used, in kg, including half of the total specimen mass and mass of superior fixturing hardware.
...
- **inf_endplate_ap_width_from_lat_view_mm**: As above, for the inferior endplate proximal to the disc.
...
- **DD_Ant_OP**: Score for presnece of anterior osteophytes, based on osteophyte size. Possible scores of 0-3.
- **DD_Endplate_Sclerosis**: Score for presence of proximal endplate sclerorsis, based on severity. Possible scores of 0-2.
- **FJ_Degen_Score**: Facet Joint degeneration score, per Walraevens et al. [1], graded using CT scans. This is the sum of scores given for hypertrophy, osteophytes, articular irregularities, and joint space narrowing. Possible scores are 0-5, where higher scores represent more degernated facet joints.
...
- **direction**: direction in which shear laods were applied, anterior or posterior.
